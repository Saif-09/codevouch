...
export interface ServiceImpact {
  ...
  asOf: string; // ISO date the entry was last checked
}

const TABLE: Record<string, ServiceImpact> = {
  clerk: {
...
export function lookupService(packageName: string): ServiceImpact | null {
  const key = PACKAGE_MAP[packageName];
  return key ? TABLE[key] : null;
}

export function allServices(): ServiceImpact[] {
