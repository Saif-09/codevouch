...
export function enableGatewayHost(host: string): void {
  gatewayHost = host;
}
...
export function assertAllowed(url: string): URL {
  const u = new URL(url);
  if (u.protocol !== 'https:') throw new Error(`egress blocked (not https): ${url}`);
  if (!isAllowedHost(u.hostname)) throw new Error(`egress blocked (host not allowlisted): ${u.hostname}`);
  return u;
}
 ...
 * (RESEARCH §6.2).
 */
export function osvBatchPost(path: string, body: unknown): Promise<any> {
