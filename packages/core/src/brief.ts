...
import { BRIEF_SCHEMA, BRIEF_SYSTEM, CONCEPTS_SCHEMA, CONCEPTS_SYSTEM } from './prompts.js';
import { redactFiles } from './redact.js';
import { fileAt, changedFiles } from './gitrepo.js';
import { clusterSession, type FeatureCluster } from './features.js';
...
async function clusterDiff(
  ...
  for (const f of kept) {
    const chunk = `--- ${f.path}\n${f.content}\n`;
    if (out.length + chunk.length > MAX_DIFF_CHARS) break;
    out += chunk;
  }
...
export async function generateBriefs(
  ...
  const session = db
    ...
    .get(sessionId) as { head_before: string; head_after: string | null } | undefined;
  if (!session?.head_after) return { created: 0, failed: 0 };
  ...
  for (const cluster of clusters) {
    ...
    let body: BriefBody;
    try {
      const { value } = await backend.run<BriefBody>({
        task: 'brief',
        system: BRIEF_SYSTEM,
    ...
    const zoneId = db
      ...
      .get(cluster.nodeIds[0]) as { zone_id: string | null } | undefined;
    ...
    db.prepare(
      `INSERT INTO nodes (id, repo_id, kind, key, label, state, alive, in_zone, critical, zone_id, first_seen_session, state_changed_at, created_at)
       VALUES (?, ?, 'decision', ?, ?, 'unknown', 1, ?, ?, ?, ?, ?, ?)`,
    ).run(id, repo.id, key, body.name, cluster.inZone ? 1 : 0, cluster.critical ? 1 : 0, zoneId?.zone_id ?? null, sessionId, now, now);
    db.prepare(
      "INSERT INTO node_states (id, node_id, from_state, to_state, cause, rep_id, at) VALUES (?, ?, NULL, 'unknown', 'ingest', NULL, ?)",
    ).run(ulid(), id, now);
    ...
    for (const artifactId of cluster.nodeIds) edge.run(id, artifactId);
    ...
    for (const label of body.concepts.slice(0, 8)) {
      ...
      if (!ckey.replace('concept:', '')) continue;
      let cid = (db.prepare("SELECT id FROM nodes WHERE repo_id = ? AND kind = 'concept' AND key = ?").get(repo.id, ckey) as { id: string } | undefined)?.id;
      if (!cid) {
        cid = ulid();
        db.prepare(
          `INSERT INTO nodes (id, repo_id, kind, key, label, state, alive, in_zone, critical, zone_id, first_seen_session, state_changed_at, created_at)
           VALUES (?, ?, 'concept', ?, ?, 'unknown', 1, ?, 0, ?, ?, ?, ?)`,
        ).run(cid, repo.id, ckey, label, cluster.inZone ? 1 : 0, zoneId?.zone_id ?? null, sessionId, now, now);
        db.prepare(
          "INSERT INTO node_states (id, node_id, from_state, to_state, cause, rep_id, at) VALUES (?, ?, NULL, 'unknown', 'ingest', NULL, ?)",
        ).run(ulid(), cid, now);
      }
