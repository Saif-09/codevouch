...
export function sweepDecay(db: Db, repoId: string): number {
  const due = db
    .prepare(
      `SELECT n.id
       FROM nodes n
    ...
    .all(repoId, DEFAULT_DECAY_DAYS) as { id: string }[];

  let moved = 0;
  for (const row of due) {
    if (applyEvent(db, row.id, { type: 'decay' }) !== null) moved++;
  }
  return moved;
}

/** Nodes whose decay window is more than 80% elapsed: the card queue. */
export function approachingDecay(db: Db, repoId: string, limit = 20): string[] {
  return (
    db
      .prepare(
        `SELECT n.id
      ...
      .all(repoId, DEFAULT_DECAY_DAYS, limit) as { id: string }[]
  ).map((r) => r.id);
}
