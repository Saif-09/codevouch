 ...
 */
export function promoteClusterArtifacts(db: Db, decisionId: string, repId: string): number {
  const artifacts = db
    .prepare(
      `SELECT a.id FROM edges e JOIN nodes a ON a.id = e.to_node
       WHERE e.from_node = ? AND e.rel = 'about' AND a.kind = 'artifact'
         AND a.alive = 1 AND a.in_zone = 1`,
    )
    .all(decisionId) as { id: string }[];

  let moved = 0;
  for (const a of artifacts) {
    // reveal first (unknown -> explained), then the pass (explained -> defended)
    applyEvent(db, a.id, { type: 'reveal' }, repId);
    if (applyEvent(db, a.id, { type: 'verdict', verdict: 'pass' }, repId) !== null) moved++;
  }
  return moved;
}
