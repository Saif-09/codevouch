...
export function renderMapSvg(model, opts) {
    ...
    const groupLabels = model.groups
        ...
        .map((g) => `<text x="${g.x + 6}" y="${g.y + g.h - 6}" font-family=${JSON.stringify(LABEL_FONT)} font-size="10" fill="${PALETTE.ink}" opacity="0.55">${esc(g.name)}</text>`)
        .join('');
    const legend = legendH === 0
        ? ''
        : legendSvg(8, mapH + 10);
...
export function legendSvg(x, y) {
    ...
    for (const [name, fill, hatch] of entries) {
        ...
        if (hatch)
            parts.push(`<rect x="${cx}" y="${y}" width="14" height="14" rx="2" fill="url(#decayhatch)"/>`);
...
export function renderShareSvg(model, repoName, gapHeadline) {
    const W = 1200;
    const H = 630;
    const statW = 380;
    const pct = model.vouchedPct;
    const inner = renderMapSvg({ ...model, groups: rescaleGroups(model, statW + 24, 24, W - statW - 48, H - 88) }, { width: W, height: H, showLegend: false })
        .replace(/^<svg[^>]*>/, '')
        .replace(/<\/svg>$/, '')
...
function rescaleGroups(model, ox, oy, w, h) {
