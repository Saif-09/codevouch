    ...
    switch (event.type) {
        ...
        case 'verdict':
            switch (event.verdict) {
                ...
                case 'fail':
                    if (state === 'defended')
                        return { to: 'explained', cause: 'rep_fail' };
...
export function applyEvent(db, nodeId, event, repId) {
    const row = db
    ...
    if (event.type === 'decay' && (row.alive !== 1 || row.in_zone !== 1))
        return null;
    const result = transition(row.state, event);
    if (!result)
        return null;
    const at = nowIso();
    db.prepare('UPDATE nodes SET state = ?, state_changed_at = ? WHERE id = ?').run(result.to, at, nodeId);
    db.prepare(`INSERT INTO node_states (id, node_id, from_state, to_state, cause, rep_id, at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run(ulid(), nodeId, row.state, result.to, result.cause, repId ?? null, at);
    return result.to;
}
/** Manual verdict override by the user; always audited as cause 'manual'. */
export function manualSetState(db, nodeId, to) {
    const row = db.prepare('SELECT state FROM nodes WHERE id = ?').get(nodeId);
    if (!row)
        throw new Error(`no such node: ${nodeId}`);
    if (row.state === to)
        return;
    ...
    db.prepare(`INSERT INTO node_states (id, node_id, from_state, to_state, cause, rep_id, at)
     VALUES (?, ?, ?, ?, 'manual', NULL, ?)`).run(ulid(), nodeId, row.state, to, at);
}
