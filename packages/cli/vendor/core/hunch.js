...
export function isSubstantivePrompt(text) {
...
export function sampled(promptId, oneIn) {
    if (oneIn <= 1)
        return true;
 ...
 */
export function checkEligibility(db, cwd, promptId, userInput, opts = {}) {
    ...
    const repo = db
        ...
        .get(cwd, cwd);
    if (!repo)
        return { eligible: false, reason: 'repo not registered' };
    const zones = db
        .prepare("SELECT COUNT(*) AS c FROM sharp_zones WHERE repo_id = ? AND stance = 'keep_sharp'")
        .get(repo.id);
    if (zones.c === 0)
        return { eligible: false, reason: 'no keep-sharp zones', repoId: repo.id };
    if (!isSubstantivePrompt(userInput))
        return { eligible: false, reason: 'prompt too slight', repoId: repo.id };
    ...
    const pending = db
        .prepare(`SELECT 1 FROM reps r JOIN nodes n ON n.id = r.node_id
    ...
    if (pending)
        return { eligible: false, reason: 'hunch already in flight', repoId: repo.id };
    const recent = db
        .prepare(`SELECT 1 FROM reps r JOIN nodes n ON n.id = r.node_id
    ...
    if (recent)
        return { eligible: false, reason: 'cooling down', repoId: repo.id };
    if (!sampled(promptId, oneIn))
        return { eligible: false, reason: 'not sampled', repoId: repo.id };
 ...
 */
export function hunchInstruction() {
    return [
...
export function openHunch(db, repoId, promptId) {
    const key = 'concept:pending-hunch';
    let nodeId = db.prepare("SELECT id FROM nodes WHERE repo_id = ? AND kind = 'concept' AND key = ?").get(repoId, key)?.id;
    const now = nowIso();
    if (!nodeId) {
        nodeId = ulid();
        db.prepare(`INSERT INTO nodes (id, repo_id, kind, key, label, state, alive, in_zone, critical, state_changed_at, created_at)
       VALUES (?, ?, 'concept', ?, 'hunch in flight', 'unknown', 1, 0, 0, ?, ?)`).run(nodeId, repoId, key, now, now);
    }
    const repId = ulid();
 ...
 */
export function recordHunch(db, input) {
    const repo = db
        .prepare('SELECT id FROM repos WHERE ? = root OR ? LIKE root || \'/%\' ORDER BY length(root) DESC LIMIT 1')
        .get(input.repoRoot, input.repoRoot);
    if (!repo)
        throw new Error('repo not registered');
    const key = `concept:${slugify(input.topic) || 'untitled'}`;
    let nodeId = db.prepare("SELECT id FROM nodes WHERE repo_id = ? AND kind = 'concept' AND key = ?").get(repo.id, key)?.id;
    const now = nowIso();
    if (!nodeId) {
        nodeId = ulid();
        db.prepare(`INSERT INTO nodes (id, repo_id, kind, key, label, state, alive, in_zone, critical, state_changed_at, created_at)
       VALUES (?, ?, 'concept', ?, ?, 'unknown', 1, 0, 0, ?, ?)`).run(nodeId, repo.id, key, input.topic.slice(0, 80), now, now);
    ...
    const open = db
        .prepare(`SELECT r.id FROM reps r JOIN nodes n ON n.id = r.node_id
       WHERE n.repo_id = ? AND r.type = 'hunch' AND r.answered_at IS NULL
    ...
    db.prepare(`UPDATE reps SET node_id = ?, answer_text = ?, verdict = ?, gap_text = ?, answered_at = ?, revealed_at = ? WHERE id = ?`).run(nodeId, input.prediction, input.matched ? 'pass' : 'fail', input.note ?? null, now, now, repId);
    return { ok: true, calibration: calibration(db, repo.id) };
}
/** Share of predictions that matched the answer's shape (spec §9). */
export function calibration(db, repoId, days = 90) {
    const row = db
        .prepare(`SELECT COUNT(*) AS n, SUM(CASE WHEN r.verdict = 'pass' THEN 1 ELSE 0 END) AS hits
