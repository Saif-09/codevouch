...
export function buildMapModel(db, repoId, width, height) {
    const rows = db
        .prepare(`SELECT id, kind, key, label, state, in_zone, critical
       FROM nodes WHERE repo_id = ? AND alive = 1`)
        .all(repoId);
    const byGroup = new Map();
    for (const n of rows) {
        const group = n.kind === 'dependency' ? 'dependencies'
    ...
    const groups = laidGroups.map((g) => {
        const inner = { x: g.x + 2, y: g.y + 2, w: Math.max(0, g.w - 4), h: Math.max(0, g.h - 4) };
        const cells = squarify(g.nodes.map((n) => ({
            id: n.id,
            label: n.label,
            kind: n.kind,
            state: n.state,
