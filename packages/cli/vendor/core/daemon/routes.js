...
import { getRepo, upsertRepo, tick, explicitStart, explicitEnd, } from '../ingest.js';
...
import { chooseBackend, metered } from '../extraction.js';
import { ulid, nowIso, mapLimit } from '../util.js';
import { DAEMON_VERSION } from './version.js';
import { readFileSync } from 'node:fs';
import { join, basename } from 'node:path';
export function createCtx(backend) {
...
function requireRepo(ctx, root) {
    if (!root)
        throw new HttpError(400, 'root required');
    const repo = getRepo(ctx.db, root);
    if (!repo)
        throw new HttpError(404, `repo not registered: ${root}. Run vouch init first.`);
    return repo;
}
/**
 ...
 */
export async function backfill(ctx, repo) {
    const db = ctx.db;
    ...
    db.prepare('INSERT INTO sessions (id, repo_id, started_at, ended_at, head_before, head_after, last_activity) VALUES (?, ?, ?, ?, ?, ?, ?)').run(sessionId, repo.id, now, now, parent, head, now);
    ...
    for (const f of files) {
        ...
        if ([...MANIFEST_FILES, ...LOCK_FILES].includes(base) && !f.includes('node_modules/')) {
            try {
    ...
    for (const dep of parseDirectDeps(map)) {
        ...
        const exists = db.prepare("SELECT id FROM nodes WHERE repo_id = ? AND kind = 'dependency' AND key = ?").get(repo.id, key);
        if (exists) {
       ...
       VALUES (?, ?, 'dependency', ?, ?, 'unknown', 1, ?, ?, ?, ?, ?, ?)`).run(id, repo.id, key, dep.name, zone?.stance === 'keep_sharp' ? 1 : 0, isCriticalDep(dep.name) ? 1 : 0, zone?.id ?? null, sessionId, now, now);
    ...
    for (const f of files) {
        ...
        for (const sym of exportedSymbols(f, content)) {
         ...
         VALUES (?, ?, 'artifact', ?, ?, 'unknown', 1, 1, ?, ?, ?, ?, ?, ?)`).run(id, repo.id, key, sym.name, zone.critical, zone.id, sym.hash, sessionId, now, now);
            db.prepare("INSERT INTO node_states (id, node_id, from_state, to_state, cause, rep_id, at) VALUES (?, ?, NULL, 'unknown', 'ingest', NULL, ?)").run(ulid(), id, now);
            artifacts++;
        }
    }
...
export async function generatePendingDossiers(ctx, repo, limit = 24, nodeIds) {
    ...
    const filter = nodeIds && nodeIds.length > 0
        ...
        : '';
    const rows = ctx.db
        .prepare(`SELECT n.id FROM nodes n
...
export const routes = [
    ...
    },
    {
        ...
        handler: async (ctx, _p, body) => {
            if (!body?.root || !body?.name)
                throw new HttpError(400, 'root and name required');
    ...
    },
    {
        ...
        handler: async (ctx, _p, _b, q) => {
            ...
            const depTotals = ctx.db.prepare(`SELECT SUM(CASE WHEN in_zone = 1 THEN 1 ELSE 0 END) AS inZone,
                SUM(CASE WHEN in_zone = 0 THEN 1 ELSE 0 END) AS outsourced
         FROM nodes WHERE repo_id = ? AND kind = 'dependency' AND alive = 1`).get(repo.id);
            return {
                repo: repo.name,
    ...
    },
    {
        ...
        handler: async (ctx, _p, body) => generatePendingDossiers(ctx, requireRepo(ctx, body?.root), body?.limit ?? 24, body?.nodeIds),
    },
    {
    ...
    },
    {
        ...
        handler: async (ctx, _p, body) => {
            ...
            if (!card)
                throw new HttpError(404, 'no card available for that node');
            return card;
        },
    },
    {
    ...
    },
    {
        ...
        handler: async (ctx, _p, body) => {
            if (!body?.nodeId)
                throw new HttpError(400, 'nodeId required');
            const question = askRep(ctx.db, body.nodeId);
            if (!question)
                throw new HttpError(404, 'node not eligible for a rep');
            return question;
        },
    },
    {
    ...
    },
    {
        ...
        handler: async (ctx, _p, _b, q) => {
            ...
            const repo = requireRepo(ctx, q.get('root') ?? undefined);
            return ctx.db
                .prepare(`SELECT n.label, n.in_zone, n.critical, z.name AS zone, (d.id IS NOT NULL) AS hasDossier, (d.body_json IS NOT NULL) AS hasBody
           FROM nodes n
    ...
    },
    {
        ...
        handler: async (ctx, _p, _b, q) => {
            ...
            const out = q.get('out');
            if (!out)
                throw new HttpError(400, 'out path required');
    ...
    },
    {
        ...
        handler: async (ctx, _p, body) => {
            ...
            let found = 0;
            for (const d of deps) {
                const [ecosystem, name] = d.key.split(/:(.+)/);
                clear.run(d.id);
    ...
    },
    {
        ...
        handler: async (ctx, _p, _b, q) => findUnused(ctx.db, requireRepo(ctx, q.get('root') ?? undefined).id),
    },
    {
        ...
        handler: async (ctx, _p, _b, q) => {
            ...
            const repo = requireRepo(ctx, q.get('root') ?? undefined);
            return ctx.db
                .prepare(`SELECT n.id, n.kind, n.label, n.state, COALESCE(z.name,'(unzoned)') AS zone,
    ...
    },
    {
        ...
        handler: async (ctx, _p, body) => {
            if (body?.confirm !== true)
                throw new HttpError(400, 'confirm: true required');
