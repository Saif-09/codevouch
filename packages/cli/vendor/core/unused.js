...
export function isConfigOnly(name) {
    return CONFIG_ONLY.some((re) => re.test(name));
}
export const CAVEAT = 'Vouch finds imports in TypeScript, JavaScript and Python source. Packages used only through config files, CLI scripts, bundler plugins or generated code have no import site and will appear here. Treat each one as a question to answer, not an instruction to delete.';
export function findUnused(db, repoId) {
    const rows = db
        .prepare(`SELECT n.id, n.label, n.in_zone,
    ...
    for (const r of rows) {
        ...
        if (r.impact_json) {
            try {
                installSizeBytes = JSON.parse(r.impact_json).installSizeBytes ?? null;
            }
            catch { /* impact is optional */ }
        }
        let ifItVanished = null;
        if (r.body_json) {
            try {
